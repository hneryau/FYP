    ...
    <head>
        ...
        <script>
		 function open_window(url){
			...
			myRef = window.open( url,'My Book','left=0,top=0,width=1050,height=height,toolbar=0,resizable=0');
		}
        </script>
    </head>
    <body>
        <?php
        $query_rs = "select book_id, bookName, category, coverPath, name from BOOK, AUTHOR where AUTHOR.author_id = BOOK.Author_id AND status = '1'";
        ...
        if ($totalRows_rs > 0) {
            while ($row = mysql_fetch_assoc($result)) {
               ...
               // echo'<td ><a target="_blank" href="controlBook.php?book_id=' . $row["book_id"] . '">View Content</a></td>';
			   ?>
