...
<script type="text/javascript">	
	...
	function increaseSectionNo(sid,cid,sno){
		var max = document.getElementById("max").value;
		// alert(max);
		if (sno >= max){}
			else{
		$.post("increaseSectionNo.php", {sid:sid, cid:cid, sno:sno},
		  function(data, success){window.location.reload(true);});
}}
	// function(data){alert(data), //debug

	function decreaseSectionNo(sid,cid,sno){
		if (sno <=1){}
			else{
		$.post("decreaseSectionNo.php", {sid:sid, cid:cid, sno:sno},
		  function(data, success){window.location.reload(true);});
}}
</script>
				...
				$chapter_id=$_GET['chapter_id'];
			        ...
			        if ($totalRows_rs > 0) {
			            while ($row = mysql_fetch_assoc($result)) {			            	
			                ...
			                echo"<td>" . $row['sectionNo'] .'
			                			<button style="float: right; width:15px" onclick="decreaseSectionNo('.$row['section_id'].','.$row['chapter_id'].','.$row['sectionNo'].')">-</button>
			                			<button style="float: right; width:15px" onclick="increaseSectionNo('.$row['section_id'].','.$row['chapter_id'].','.$row['sectionNo'].')">+</button>' ."</td>";
